...
class Agent:
    def __init__(self, num_states, num_actions, continuous,
        ...
        if self.continuous:
            ...
            self.action_buffer = np.zeros(
                (self.buffer_capacity, disc_actions_num))
        self.reward_buffer = np.zeros((self.buffer_capacity, 1))
        self.next_state_buffer = np.zeros((self.buffer_capacity, num_states))
        self.done_buffer = np.zeros(
            (self.buffer_capacity, 1)).astype(np.float32)
        ...
        self.ou_noise = OUActionNoise(mean=np.zeros(1), std_deviation=float(
            std_dev) * np.ones(1), theta=theta, dt=dt)

        self.actor_model = get_actor(
            num_states, num_actions, continuous, disc_actions_num, actor_layer_1, actor_layer_2)
        self.critic_model = get_critic(
            num_states, num_actions, continuous, disc_actions_num, critic_layer_1, critic_layer_2)
    ...
    def update(self, state_batch, action_batch, reward_batch, next_state_batch, done_batch, loss_func):
        with tf.GradientTape() as tape:
            ...
            y = reward_batch + done_batch * self.gamma * \
                self.target_critic(
        ...
        critic_capped_grad = [(tf.clip_by_value(
            grad, clip_value_min=-1, clip_value_max=1), var) for grad, var in critic_gvd]
        ...
        actor_capped_grad = [(tf.clip_by_value(
            grad, clip_value_min=-1, clip_value_max=1), var) for grad, var in actor_gvd]
    ...
    def learn(self):
        ...
        self.update(state_batch, action_batch, reward_batch,
                    next_state_batch, done_batch, self.loss_func)
